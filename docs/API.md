...
Provider = { id: "claude"|"openai"|"gemini", label, connected: boolean,
             source: "oauth"|"api-key"|null, note?: string,
             roles: ("edit"|"chat"|"image"|"video")[], models: [{ id, label }] }
```
...
PUT  /api/connections/:provider/key      { apiKey } → 200 — lưu API key ngoài repo
POST /api/connections/:provider/test     → { ok, message? } — gọi thử API để kiểm tra key
```
provider ∈ `claude` | `gemini` | `openai` | `soniox`.

Key nhập từ UI được ghi vào `~/.aiev/credentials.env` (hoặc `AIEV_CREDENTIALS_FILE`), mode 0600,
không ghi vào repo. Response chỉ có key đã che. File `.env` trong repo vẫn được đọc để tương thích
bản cũ, nhưng không phải vị trí mặc định cho secret mới.
...
```
GET  /api/omni-video → OmniVideoItem[]
POST /api/omni-video multipart:
     prompt, task=text_to_video|image_to_video|reference_to_video|edit,
     aspect=16:9|9:16, source? (ảnh <=25MB, video <=250MB), previousInteractionId?
     → 201 { id, interactionId, file, relPath, mediaUrl, model, task, aspect }
```

Dùng `gemini-omni-flash-preview` qua Interactions API. Video nguồn được upload qua Files API và
xóa sau lượt xử lý; `previousInteractionId` cho phép chỉnh tiếp bằng hội thoại. Output lớn dùng
URI delivery, poll Files API đến ACTIVE rồi tải MP4 vào `outputs/`.
Edit video upload có thể không khả dụng ở mọi khu vực do giới hạn preview của Google.

## Nhận diện sản phẩm local

```
GET  /api/product-settings → { appName, source, logoFile, logoUrl }
PUT  /api/product-settings { appName?, source? } → settings mới
POST /api/product-settings/logo multipart logo (PNG/JPG/SVG <=5MB) → settings mới
GET  /api/product-settings/logo → file logo
```

Metadata và logo nằm trong `apps/server/data/` (bị ignore và bị loại khỏi safe ZIP). SVG được phục
vụ với CSP sandbox. Mặc định source là `Nguồn: Nguyễn Văn Hoàng`.
...
- **Migration tự động**: lần chạy đầu, `assets/brand/design-system.json` (nếu có) chuyển thành style
  đầu tiên (id "aiev-brand", tag ["brand"]) và làm default. `/api/design-system` BỎ HẲN.
