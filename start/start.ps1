# AIEV - Mr Hoàng - script khởi động
...
# Đồng bộ tiêu đề terminal với tên người dùng đã lưu trong Cấu hình. Bản cài
# mới chưa có settings thì dùng tên trung tính, không còn đóng cứng thương hiệu.
$appName = "AIEV Local Studio"
try {
    $productFile = Join-Path $root "apps\server\data\product-settings.json"
    if (Test-Path -LiteralPath $productFile) {
        $savedName = (Get-Content -LiteralPath $productFile -Raw | ConvertFrom-Json).appName
        if ($savedName -is [string] -and -not [string]::IsNullOrWhiteSpace($savedName)) {
...
# 3. Cài dependencies lần đầu hoặc sửa một lần cài dở dang. Chỉ kiểm tra
# node_modules là chưa đủ: npm có thể đã tạo thư mục rồi mới lỗi giữa chừng.
$dependencyMarkers = @(
...
foreach ($marker in $dependencyMarkers) {
...
if (-not $dependenciesReady) {
    Write-Step "Đang cài hoặc sửa dependencies còn thiếu (vài phút)..."
