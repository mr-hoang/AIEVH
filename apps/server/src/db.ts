...
const db = new DatabaseSync(path.join(paths.dataDir, "app.sqlite"));
...
export function listJobs(limit = 50, projectId?: string): JobRow[] {
  ...
  if (projectId) {
    return db
      ...
      .all(projectId, capped) as unknown as JobRow[];
  }
  return db
...
export function usageByModel(days?: number): UsageByModelRow[] {
  ...
  const rows = db
    ...
    .all(...params) as unknown as UsageByModelRow[];
  return rows.map((r) => ({
    provider: r.provider,
    model: r.model ?? null,
...
export function setChatSessionModelEffort(
  ...
  const providerFamily = (value: string | null | undefined): "openai" | "claude" =>
    value?.startsWith("gpt-") || value?.startsWith("codex-") ? "openai" : "claude";
  const providerChanged =
    model !== undefined && current !== undefined && providerFamily(model) !== providerFamily(current.model);
  const sets: string[] = [];
  const values: SQLInputValue[] = [];
  if (model !== undefined) {
    sets.push("model = ?");
  ...
  // Session IDs của Codex và Claude không tương thích. Khi người dùng đổi
  // provider, lượt kế tiếp phải tạo thread mới thay vì resume nhầm thread cũ.
  if (providerChanged) {
    db.prepare("UPDATE chat_sessions SET sdkSessionId = NULL WHERE sessionId = ?").run(sessionId);
  }
}
