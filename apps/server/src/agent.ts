...
import { createCodexQuery, type CodexQuery } from "./codexAgent.js";
import { createGeminiQuery, type GeminiQuery } from "./geminiAgent.js";
import { hasClaudeAuth, hasCodexAuth, hasCodexCli, hasGeminiAuth, hasGeminiCli, paths, repoRoot } from "./config.js";
import * as db from "./db.js";
import { broadcast } from "./events.js";
import { normOutput, readMeta } from "./meta.js";
import { readRenderSettings } from "./renderSettings.js";
import { isCodexModel, isGeminiModel } from "./routes/providers.js";
...
type AgentQuery = Query | CodexQuery | GeminiQuery;
const running = new Map<string, AgentQuery>();
...
function hasAgentAuth(model?: string | null): boolean {
  if (isCodexModel(model)) return hasCodexAuth() && hasCodexCli();
  if (isGeminiModel(model)) return hasGeminiAuth() && hasGeminiCli();
...
export async function runAgent(
  ...
  const session = db.getChatSession(sessionId);
  const useCodex = isCodexModel(session?.model);
  const useGemini = isGeminiModel(session?.model);
  const hasSelectedAuth = hasAgentAuth(session?.model);
  if (!hasSelectedAuth) {
    emit({
      sessionId,
      kind: "error",
      error: useCodex
        ? "Chưa có xác thực ChatGPT/Codex. Cài Codex CLI rồi chạy `codex login`, hoặc thêm OPENAI_API_KEY dự phòng ở trang Kết nối."
        : useGemini
          ? "Chưa có xác thực Gemini. Cài Gemini CLI rồi đăng nhập Google, hoặc thêm GEMINI_API_KEY ở trang Kết nối."
          : "Chưa có xác thực Claude. Đăng nhập Claude Code trên máy này hoặc thêm ANTHROPIC_API_KEY dự phòng ở trang Kết nối.",
    });
  ...
  try {
    q = useCodex
      ...
      : useGemini
        ? createGeminiQuery({ prompt, sessionId: sdkSessionId, model: session?.model })
        : query({
          prompt,
        ...
        try {
          ...
          if (inTok > 0 || outTok > 0 || cost > 0) {
            db.addTokenUsage(
              ...
              outTok,
              cost,
              useCodex ? "openai" : useGemini ? "gemini" : "claude",
            );
          }
        } catch {
...
export async function autoResumeStartup(): Promise<void> {
  for (const id of db.startupInterruptedSessions) {
    ...
    if (!hasAgentAuth(session.model)) continue;
