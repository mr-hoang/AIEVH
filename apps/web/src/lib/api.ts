...
export type ProviderRole = "edit" | "chat" | "image" | "video";
...
export interface Provider {
  ...
  label: string;
  connected: boolean;
  /** oauth = subscription CLI; api-key = key dự phòng ngoài repo. */
...
async function ensureToken(): Promise<string | null> {
  ...
  tokenPromise = (async () => {
    ...
    // Token trong localStorage có thể thuộc lần chạy/bản cài cũ. Trên máy chủ,
    // luôn hỏi health trực tiếp trước để tự thay token cũ; chỉ dùng bản đã lưu
    // làm fallback khi mở dashboard từ xa và không gọi được port 6869.
    ...
    if (stored) {
      persistToken(stored); // fallback cho dashboard mở từ xa/tunnel
      return stored;
    }
...
export const createThumbnail = (
  id: string,
  input: { title: string; frameAt?: number; bgPrompt?: string; imageProvider?: "openai" | "gemini" }
) =>
...
export const uploadBriefReference = (id: string, file: File) => {
  const form = new FormData();
  form.append("file", file);
  return request<Brief>(`/api/projects/${encodeURIComponent(id)}/brief/reference-file`, {
    method: "POST",
    body: form,
  });
};

export const importBriefGoogleSheet = (id: string, url: string) =>
  post<Brief>(`/api/projects/${encodeURIComponent(id)}/brief/reference-link`, { url });

export const deleteBriefReference = (id: string) =>
  request<Brief>(`/api/projects/${encodeURIComponent(id)}/brief/reference`, { method: "DELETE" });

/** Gán thứ tự dựng và vai trò cho clip; sceneOrder null = tự động từ trên xuống. */
export const updateAssetSequence = (
  projectId: string,
  file: string,
  ...
  jsonBody<FileInfo>(
    `/api/projects/${encodeURIComponent(projectId)}/assets/${encodeURIComponent(file)}/sequence`,
    "PUT",
    { sceneOrder, sceneRole }
  );
...
export interface OmniVideoItem {
  id: string;
...
export const getOmniVideos = () => request<OmniVideoItem[]>("/api/omni-video");

export function createOmniVideo(input: {
  prompt: string;
  task: OmniVideoTask;
  aspect: OmniAspect;
  source?: File | null;
  ...
  if (input.previousInteractionId) {
    form.append("previousInteractionId", input.previousInteractionId);
  }
  return request<OmniVideoItem>(`${serverOrigin()}/api/omni-video`, {
    method: "POST",
    body: form,
  });
}

// ============ Nhận diện ứng dụng local ============
...
export const getProductSettings = () => request<ProductSettings>("/api/product-settings");
export const updateProductSettings = (input: { appName: string; source: string }) =>
  jsonBody<ProductSettings>("/api/product-settings", "PUT", input);
export function uploadProductLogo(file: File) {
  const form = new FormData();
  form.append("logo", file);
  return request<ProductSettings>(`${serverOrigin()}/api/product-settings/logo`, {
    method: "POST",
    body: form,
  });
}
